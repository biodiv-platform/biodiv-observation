...
public class ConstructESDocument {
	...
	public ObservationESDocument getESDocumentStub(String observationId) {
		...
		String qry = "SELECT id observation_id, author_id, created_by, profile_pic, created_on, group_id, group_name, CONCAT(group_id,'|',group_name) sgroup_filter, "
				+ "row_to_json((SELECT t FROM (SELECT latitude AS lat, longitude as lon)t))\\:\\:text AS location, "
				+ "location_information\\:\\:text AS location_information, " + "notes, from_date,  "
				...
				+ "LEFT OUTER JOIN "
				+ "(SELECT longitude lon , latitude lat, location_information FROM extended_location_information) L ON L.lon = O.longitude  "
				...
				+ "	t))\\:\\:jsonb recos " + "FROM " + "	( " + "	SELECT  " + "	observation_id, recommendation_id, "
				...
				+ "				custom_field_aggregation, custom_field_filter)f)) custom_field_values WHERE u_custom_field_id is not null)t)))) custom_field  "
				+ " " + "			FROM "
				...
				+ "			INNER JOIN " + "			(SELECT id ,icon, name, webaddress FROM user_group ) U "
