...
public class ConstructESDocument {
	...
	public List<ObservationESDocument> getESDocumentStub(String observationId) {
		...
		String qry = "SELECT id observation_id, author_id, created_by, profile_pic, created_on, group_id, group_name, CONCAT(group_id,'|',group_name,'|',group_order) sgroup_filter, "
				...
				+ "END AS no_media, " + "CASE  " + " WHEN O.is_locked = true  OR  is_verified = true THEN true "
				...
				+ "  (taxon_detail->'status') AS taxonstatus,(taxon_detail->'italicised_form') AS italicised_form, accepted_name_id, accepted_name "
				...
				+ "        longitude IS NOT NULL AND  (O.is_locked = true OR reco_vote_count >= 2)  AND "
				+ "        flag_count = 0 AND"
				...
				+ "no_of_identifications, data_table_id, date_accuracy, is_verified,basis_of_record FROM observation where is_deleted IS NOT TRUE AND id in ( "
				+ observationId + " )) O " + "LEFT OUTER JOIN  "
				+ "(SELECT id r_id, file_name AS repr_image_url FROM resource) I ON I.r_id = O.repr_image_id   "
				+ "LEFT OUTER JOIN"
				+ "(SELECT id dt_id,dataset_id data_set_id, title as data_table_title,basis_of_data FROM data_table) DT ON O.data_table_id = DT.dt_id "
				+ "LEFT OUTER JOIN"
				...
				+ "END AS taxon_detail,  " + "(reco_vote->>'accepted_name_id')\\:\\:bigint AS accepted_name_id, " + "all_reco_vote ,Count AS reco_vote_count " + "FROM " + " " + "(SELECT  "
				...
				+ "	reco_vote, " + " "
				+ "	row_to_json(( SELECT t FROM (SELECT recommendation_id, common_names, scientific_name, hierarchy, taxon_detail, accepted_name_id)  "
				...
				+ "		MAX(accepted_name_id)accepted_name_id,	sum(Count) Count  	 " + "	FROM  " + "	(SELECT "
				+ "	observation_id, recommendation_id, common_name_reco_id, Count,  "
				...
				+ "		profile_pic)t))) " + "		ELSE NULL END AS authors_voted, "
				...
				+ "		normalized_form AS scientific_name, rank, status, italicised_form, " + "		position )t))) "
				+ "		ELSE NULL " + "	END AS taxon_detail " + "		 " + "	FROM  " + "		(  "
				+ "		SELECT  "
				+ "		observation_id, id reco_vote_id, author_id voted_by_author_id, voted_by , recommendation_id, "
				+ "		common_name_reco_id, voted_on, taxon_concept_id, last_modified, profile_pic, recommendation_name, "
				+ "		CASE  "
				...
				+ "		FROM  "
				+ "		(SELECT id, observation_id, author_id , recommendation_id, common_name_reco_id, voted_on, 1\\:\\:integer AS Count "
				...
				+ "			END AS common_name, language_id, is_scientific_name, accepted_name_id, Count"
				+ "		FROM  " + "		(SELECT  "
				+ "		id, observation_id, author_id, recommendation_id, common_name_reco_id, voted_on,0\\:\\:integer AS Count  "
				...
				+ "		position    " + "		FROM "
				+ "		( SELECT id t_id, canonical_form, normalized_form, italicised_form, name, rank, status, position "
				...
				+ "LEFT OUTER JOIN (SELECT id an_id, name accepted_name FROM taxonomy_definition WHERE is_deleted = false) AN ON ORV.accepted_name_id = AN.an_id "
				+ "LEFT OUTER JOIN "
				+ "		( " + "		SELECT  observation_id ,  " + "		jsonb_agg( DISTINCT  "
				+ "		to_jsonb((row_to_json((SELECT t FROM (SELECT resource_id id, description , file_name , type ,url, rating , upload_time , uploader_id,contributor,license_id,license_name,license_url,x, y, width, height, selection_status) t ))  "
				...
				+ "			 		CONCAT(name,'|',trait_id,'|',value, '|', trait_value_id)  "
				+ "			 	WHEN fact_value IS NOT NULL THEN  " + "					CASE "
				+ "						WHEN to_value IS NOT NULL THEN  "
				+ "							CONCAT(name,'|',trait_id,'|',fact_value,'-',to_value)  "
				+ "						ELSE " + "							CONCAT(name,'|',trait_id,'|',fact_value) "
				+ "					END " + "				ELSE "
				+ "					CONCAT(name,'|',trait_id,'|',from_date,'-',to_date) "
				+ "			END AS trait_aggregation , "

				+ "CASE " + "				WHEN value IS NOT NULL THEN  "
				+ "					CONCAT(trait_id,'|',name,'|',trait_types,'|',value,'_',trait_value_id,'|',COALESCE(tv_icon,'0')) "
				+ "				WHEN fact_value IS NOT NULL THEN  " + "					CASE "
				+ "						WHEN to_value IS NOT NULL THEN  "
		...
		try {
			...
			if (pullFunction.equalsIgnoreCase("null_pull_function")) {
				finalQuery = qry.replace("public.null_pull_function(lon, lat)", "null");
			}
			Query<ObservationESDocument> query = session.createNativeQuery(finalQuery, ObservationESDocument.class);
